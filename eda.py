...
def eda_page():
    ...
    if dataset_name:
        ...
        x_axis = st.selectbox("Pilih Kolom untuk Sumbu X:", options=data.columns)
        y_axis = st.selectbox("Pilih Kolom untuk Sumbu Y:", options=data.columns)
        plot_type = st.selectbox("Pilih Tipe Visualisasi:", ["Scatter Plot", "Bar Plot", "Line Plot", "Pie Chart"])
        top_n = st.selectbox("Tampilkan Berapa Banyak Data:", ["Top 5", "Top 10", "Semua"], index=2)

        grouped_data = data.groupby(x_axis)[y_axis].sum().reset_index()
        if pd.api.types.is_numeric_dtype(grouped_data[y_axis]):
            if top_n == "Top 5":
                grouped_data = grouped_data.nlargest(5, y_axis)
            elif top_n == "Top 10":
                grouped_data = grouped_data.nlargest(10, y_axis)

        if st.button("Buat Visualisasi"):
            plt.figure(figsize=(12, 6))
            if plot_type == "Scatter Plot":
                sns.scatterplot(x=grouped_data[x_axis], y=grouped_data[y_axis])
            elif plot_type == "Bar Plot":
                sns.barplot(x=grouped_data[x_axis], y=grouped_data[y_axis], hue=grouped_data[x_axis], palette='viridis', legend=False)
            elif plot_type == "Line Plot":
                sns.lineplot(x=grouped_data[x_axis], y=grouped_data[y_axis])
            elif plot_type == "Pie Chart":
                plt.pie(grouped_data[y_axis], labels=grouped_data[x_axis], autopct='%1.1f%%', colors=sns.color_palette('viridis', len(grouped_data)))
                plt.title(f"{plot_type} - {x_axis} vs {y_axis}")
        ...
        # Distribusi Data
        st.subheader("4. Distribusi Data")
        ...
        st.subheader("5. Deteksi dan Penanganan Outlier")
        with st.expander('**Apa itu outlier?**'):
            st.info('Outlier atau *pencilan* adalah data yang berbeda secara signifikan dari data lain dalam suatu sampel/populasi. Data outlier dapat muncul karena kesalahan pengukuran, kesalahan input data, atau karena adanya anomali dalam data yang diukur.')
        ...
        if st.button("Simpan Hasil"):
            if not isinstance(data, pd.DataFrame):
                ...
                st.success(f"silakan lanjutkan ke halaman Clustering di Menu 🏃🏼‍♂️‍➡️")
